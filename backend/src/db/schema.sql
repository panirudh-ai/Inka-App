...
CREATE TABLE IF NOT EXISTS users (
  ...
  role TEXT NOT NULL CHECK (
    role IN ('admin', 'project_manager', 'engineer', 'client')
  ),
...
ALTER TABLE projects
ADD COLUMN IF NOT EXISTS client_id UUID REFERENCES clients(id) ON DELETE
SET NULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_single_open_cr_per_project ON change_requests(project_id)
...
ADD COLUMN IF NOT EXISTS floor_label TEXT NOT NULL DEFAULT 'Unassigned';
ALTER TABLE project_bom_items
...
CREATE INDEX IF NOT EXISTS ix_items_tenant_category_pt_brand ON items(
  tenant_id,
  category_id,
  product_type_id,
  brand_id
