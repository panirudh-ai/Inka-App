    ...
    <Fade in timeout={420}>
      ...
      >
        ...
        <Grid container spacing={2.2}>
          ...
          <Grid size={{ xs: 12, sm: 6, lg: 3 }}><KpiCard title="Open CRs" value={openCrCount} subtitle="Single CR governance" color={G8.orange} /></Grid>
          <Grid size={{ xs: 12, sm: 6, lg: 3 }}><KpiCard title="Categories" value={categories.length} subtitle="Structured scope hierarchy" color={G8.orange} /></Grid>
          <Grid size={{ xs: 12, sm: 6, lg: 3 }}><KpiCard title="Item Master" value={items.length} subtitle="Model-level control" color={G8.orange} /></Grid>
        </Grid>
        )}
        ...
        {tab === 0 && (
          ...
          {viewMode === "dashboard" && selectedProjectId && selectedProjectDashboard?.project?.id === selectedProjectId && (
            <Paper id="admin-project-dashboard" sx={{ p: 0, mt: 2, borderRadius: 3, overflow: "hidden", border: "1px solid", borderColor: "divider" }}>
              ...
              <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5, px: 2.5, py: 2, borderBottom: "1px solid", borderColor: "divider" }}>
                {[
                  { label: "Change Request", value: adminOpenCr ? "Open" : "Clear", sub: adminOpenCr ? "Pending approval" : "All approved", color: G8.orange },
                  { label: "Total Scope", value: `₹${Number(selectedProjectDashboard.summary?.total_scope_value || 0).toLocaleString()}`, sub: "Approved BOM value", color: G8.orange },
                  { label: "Delivered", value: `₹${Number(selectedProjectDashboard.summary?.total_delivered_value || 0).toLocaleString()}`, sub: "Value on-site", color: G8.orange },
                  { label: "Balance", value: `₹${Number(selectedProjectDashboard.summary?.total_balance_value || 0).toLocaleString()}`, sub: "Remaining value", color: G8.orange },
                  { label: "Site Visits", value: Number(selectedProjectDashboard.summary?.visit_count || 0), sub: "Total logged", color: G8.orange },
              ...
              {projectDetailsLoading ? (
                ...
                <Paper sx={{ mt: 2, p: 2, borderRadius: 2, border: "1px solid", borderColor: "divider" }}>
                  ...
                  <Grid container spacing={1.5}>
                    <Grid size={{ xs: 12, md: 3 }}><Typography sx={{ fontSize: "0.62rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "text.secondary", mb: 0.3 }}>Project</Typography><Typography variant="body2" sx={{ fontWeight: 600 }}>{selectedProjectDashboard.project.name}</Typography></Grid>
                    <Grid size={{ xs: 12, md: 3 }}><Typography sx={{ fontSize: "0.62rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "text.secondary", mb: 0.3 }}>Client</Typography><Typography variant="body2" sx={{ fontWeight: 600 }}>{selectedProjectDashboard.project.client_name}</Typography></Grid>
                    <Grid size={{ xs: 12, md: 3 }}><Typography sx={{ fontSize: "0.62rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "text.secondary", mb: 0.3 }}>Location</Typography><Typography variant="body2" sx={{ fontWeight: 600 }}>{selectedProjectDashboard.project.location}</Typography></Grid>
                    <Grid size={{ xs: 12, md: 3 }}>
                      <Typography variant="caption" color="text.secondary">Status</Typography>
                      <Select
                        size="small"
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1.5 }}>
                    <Box sx={{ width: 4, height: 20, borderRadius: 2, bgcolor: G8.orange }} />
                  ...
                  <Stack spacing={0.8} sx={{ mt: 0.8 }}>
                    {adminProjectContacts.map((c, idx) => (
                      <Stack key={`contact-${idx}`} direction={{ xs: "column", md: "row" }} spacing={1}>
                        ...
                        <Tooltip title="Save contacts"><IconButton size="small" color="success" onClick={() => adminSaveContacts().catch(() => setAdminToast({ open: true, severity: "error", text: "Save contacts failed" }))}><CheckCircleOutlineIcon fontSize="small" /></IconButton></Tooltip>
                  ...
                  <Stack spacing={0.8} sx={{ mt: 1.2 }}>
                    {adminVisits.slice(0, 5).map((v) => (
                      <Box key={v.id} sx={{ display: "flex", gap: 1.2, alignItems: "flex-start" }}>
                        <Box sx={{ width: 8, height: 8, borderRadius: "50%", bgcolor: G8.orange, mt: "5px", flexShrink: 0 }} />
                        <Box>
                  ...
                  <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1.5, mb: 2 }}>
                    {[
                      { label: "Total Visits", value: Number(selectedProjectVisitSummary?.totals?.total_visits || 0), color: G8.orange },
                      { label: "Engineers", value: Number(selectedProjectVisitSummary?.totals?.engineer_count || 0), color: G8.orange },
                      { label: "First Visit", value: selectedProjectVisitSummary?.totals?.first_visit_date ? new Date(selectedProjectVisitSummary.totals.first_visit_date).toLocaleDateString() : "—", color: G8.orange },
                      { label: "Last Visit", value: selectedProjectVisitSummary?.totals?.last_visit_date ? new Date(selectedProjectVisitSummary.totals.last_visit_date).toLocaleDateString() : "—", color: G8.orange },
                      ...
                      <Paper sx={{ p: 1.8, borderRadius: 2, border: "1px solid", borderColor: "divider" }}>
                        ...
                        <Stack spacing={1}>
                          {(selectedProjectVisitSummary?.byEngineer || []).map((r) => {
                            const maxV = Math.max(...(selectedProjectVisitSummary?.byEngineer || []).map((x) => x.visit_count), 1);
                            return (
                              <Box key={r.engineer_id || r.engineer_name}>
                                <Stack direction="row" justifyContent="space-between" sx={{ mb: 0.3 }}>
                                  <Typography variant="body2" sx={{ fontWeight: 500 }}>{r.engineer_name}</Typography>
                                  <Typography variant="body2" sx={{ fontWeight: 700, color: G8.orange }}>{r.visit_count}</Typography>
                                </Stack>
                                <Box sx={{ height: 6, borderRadius: 3, bgcolor: (t) => t.palette.mode === "dark" ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.07)", overflow: "hidden" }}>
                                  <Box sx={{ height: "100%", width: `${(r.visit_count / maxV) * 100}%`, bgcolor: G8.orange, borderRadius: 3, transition: "width 0.5s ease" }} />
                                </Box>
                              </Box>
                            );
                          })}
                      ...
                      <Paper sx={{ p: 1.8, borderRadius: 2, border: "1px solid", borderColor: "divider" }}>
                        ...
                        <Stack spacing={1}>
                          {(selectedProjectVisitSummary?.byMonth || []).map((r) => {
                            const maxV = Math.max(...(selectedProjectVisitSummary?.byMonth || []).map((x) => x.visit_count), 1);
                            return (
                              <Box key={r.month_key}>
                                <Stack direction="row" justifyContent="space-between" sx={{ mb: 0.3 }}>
                  ...
                  <Stack spacing={0}>
                    {selectedProjectActivity.slice(0, 10).map((a, idx) => {
                      ...
                      return (
                        <Box key={a.id} sx={{ display: "flex", gap: 1.5, position: "relative" }}>
                          <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
                            ...
                            {idx < Math.min(selectedProjectActivity.length, 10) - 1 && <Box sx={{ width: 2, flex: 1, bgcolor: (t) => t.palette.mode === "dark" ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.1)", my: 0.3 }} />}
                          </Box>
            ...
            {adminInnerTab === 0 && (
              <Zoom in timeout={350}>
                <Box sx={{ mt: 2 }}>
                  ...
                  {(selectedProjectDashboard?.bom?.length > 0) && (
                    <Paper sx={{ p: 2, mb: 2, border: "1px solid", borderColor: "divider" }}>
                      <PipelineSummary bom={selectedProjectDashboard.bom} />
                    </Paper>
                  )}
                  {adminGrouped.map(([category, rows]) => (
      ...
      <Dialog open={editProjectOpen} onClose={() => setEditProjectOpen(false)} fullWidth maxWidth="sm" PaperProps={{ sx: { borderRadius: 3 } }}>
        ...
        <DialogActions sx={{ px: 3, py: 2, borderTop: "1px solid", borderColor: "divider", gap: 1 }}>
          ...
          <Button variant="contained" startIcon={<CheckCircleOutlineIcon />} onClick={() => saveAdminEditProject().catch(() => setNotice("Update project failed"))} sx={{ bgcolor: G8.orange, "&:hover": { bgcolor: "#c44a3c" }, borderRadius: 2, px: 3 }}>Save Changes</Button>
        </DialogActions>
      </Dialog>
